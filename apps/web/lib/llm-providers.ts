...
export class OpenAIProvider implements LLMProvider {
  ...
  async complete({ agentId, query, memories, persona }: { agentId: AgentId; query: string; memories: Memory[]; persona?: string }): Promise<string> {
    if (memories.length === 0) return REFUSAL;
    const res = await this.client.chat.completions.create({
      model: "gpt-4o",
      max_tokens: 512,
      messages: [
        { role: "system", content: systemPrompt(agentId, memories, persona) },
...
export class AnthropicProvider implements LLMProvider {
  ...
  async complete({ agentId, query, memories, persona }: { agentId: AgentId; query: string; memories: Memory[]; persona?: string }): Promise<string> {
    ...
    const res = await this.client.messages.create({
      ...
      max_tokens: 512,
      system: systemPrompt(agentId, memories, persona),
